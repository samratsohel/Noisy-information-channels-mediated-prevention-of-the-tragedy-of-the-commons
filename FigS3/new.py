...
for ax, d in zip(axs.flat, [cooperation11, cooperation12, cooperation13, efficacy11, efficacy12, efficacy13, abundant11, abundant12, abundant13]):
    ...
    if row == 3:
        levels = 3
        contours = ax.contourf(data, extent=[0, 1.0, 1.0, 0.0], colors='none', hatches=['..', '--', '\\\\'], levels=7)
        contours = ax.contour(data, extent=[0, 1.0, 1.0, 0.0], colors='black', alpha=1.0, levels=levels)
    ...
    subfig_label = fr"($\rm {chr(97 + count - 1)}$)"
    ax.annotate(subfig_label, xy=(-0.4, 0.9), xycoords='axes fraction', fontsize=23, fontweight='bold')

# Add the patch labels under the last row
